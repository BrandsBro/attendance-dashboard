    ...
    <div className="payroll-modal-backdrop" onClick={onClose}>
      <div className="payroll-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="tabs" style={{ marginBottom: 16 }}>
          <button className={`tab ${tab === 'designations' ? 'active' : ''}`} onClick={() => setTab('designations')}>Designations</button>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
          <input
            className="input"
            style={{ flex: 1 }}
            placeholder={`Add new ${tab === 'designations' ? 'designation' : 'department'}…`}
            value={newVal}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 300, overflowY: 'auto' }}>
          {list.map(v => (
            <div key={v} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 10px', background: 'var(--bg)', borderRadius: 6, border: '1px solid var(--border)' }}>
